...
async function removeFactionSuperscript(itemNameImage: Mat): Promise<Mat> {
  const resizedMask = await resizeHeightTo(itemNameImage.rows * 1.35, factionFlagMask, INTER_NEAREST);
  ...
  const paddedMask = await croppedMask.copyMakeBorderAsync(/* top= */ 0, /* bottom= */ 0, maskLeftPadding, /* right= */ 0, BORDER_CONSTANT, black);
